    ...
    {#each Object.entries($steps) as [idx, step]}
      ...
      <div class="relative z-10 flex flex-1 items-center">
        ...
        <p class="ml-3 text-sm font-medium
                   { $currentStep >= i ? 'text-primary' : 'text-muted' }">
                   {t(`steps.${step.name}`)}
        </p>
        ...
        {#if i < $totalSteps}
          <div
            class="mx-4 flex-1 border-t-2
                   { $currentStep > i ? 'border-primary-500' : 'border-muted' }">
          </div>
        {/if}
      </div>
    {/each}
